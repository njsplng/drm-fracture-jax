...
def generate_mesh() -> None:
    ...
    p2 = geo.addPoint(0.0, 1.0, 0, lengthscale_parameter)
    p3 = geo.addPoint(4.5, 1.0, 0, lengthscale_parameter)
    p4 = geo.addPoint(4.5, 0.0, 0, lengthscale_parameter)
    ...
    f1 = field.add("Box")
    field.setNumber(f1, "VIn", lengthscale_parameter / 10)  # Min mesh size
    field.setNumber(f1, "VOut", lengthscale_parameter)  # Max mesh size
    field.setNumber(f1, "XMin", 1.8)
    field.setNumber(f1, "XMax", 2.7)
    field.setNumber(f1, "YMin", 0.0)
    field.setNumber(f1, "YMax", 1.0)
